...
class VelocityConfigManager(private val plugin: PluginMain) : BaseConfigManager() {

    init {
        initConfig(plugin.dataDirectory.toFile(), "config.yml")
    }
    ...
    override fun getConfig(name: String): YamlConfiguration {
        ...
        try {
            ...
            plugin.logger.warn("无法加载默认配置文件: " + e.message)
        }
    ...
    override fun createDefaultConfig(name: String?) {
        ...
        if (!file.exists()) {
            try {
                ...
                plugin.logger.warn("无法创建默认配置文件: " + e.message)
            }
        }
    }
    ...
    override fun saveConfig(name: String?) {
        ...
        try {
            ...
            plugin.logger.warn("保存配置文件失败: " + e.message)
        }
    }
